...
% $Id: callquadprog.m,v 1.14 2006/12/07 09:57:28 joloef Exp $
...
if nnz(Q) == 0
    ...
    if options.saveduals
        ...
        lambda = [];
        [x,fmin,flag,output] = linprog(c, A, b, Aeq, beq, lb, ub, x0,options.quadprog);                
    end
else   
    if options.saveduals
        ...
        lambda = [];
        [x,fmin,flag,output] = quadprog(2*Q, c, A, b, Aeq, beq, lb, ub, x0,options.quadprog);    
    end
    if flag==5

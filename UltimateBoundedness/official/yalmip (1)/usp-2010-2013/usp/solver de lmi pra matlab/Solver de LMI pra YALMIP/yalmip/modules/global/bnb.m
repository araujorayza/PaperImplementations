...
% $Id: bnb.m,v 1.23 2006/12/08 10:11:40 joloef Exp $
...
while ~isempty(node) & (solved_nodes < p.options.bnb.maxiter) & (isinf(lower) | gap>p.options.bnb.gaptol)
    ...
    if any(p.ub<p.lb - 1e-12)
        ...
        % SOME HACKS TO TEST BUGS IN BPMPD
%                 if output.problem == 11
%                     relaxed_p.options.bpmpd.opts(64) = 0;
%                     relaxed_p.options.bpmpd.opts(38) = 3;
%                     output = bnb_solvelower(lowersolver,relaxed_p,upper+abs(upper)*1e-2+1e-4,lower);
%                     if output.problem == 11
%                         relaxed_p.options.bpmpd.opts(64) = 0;
%                         relaxed_p.options.bpmpd.opts(38) = 4;
%                         output = bnb_solvelower(lowersolver,relaxed_p,upper+abs(upper)*1e-2+1e-4,lower);
%                         if output.problem == 11
%         
%                             pp =relaxed_p;
%                             pp.lb(isinf(pp.lb)) = -100;
%                             pp.ub(isinf(pp.ub)) = 100;
%                             output = bnb_solvelower(lowersolver,pp,upper+abs(upper)*1e-2+1e-4,lower);
%                             if output.problem == 11
%                                 disp('Lower bound failed')
%                                 error
%                                 %              output = bnb_solvelower('callclp',relaxed_p,upper+abs(upper)*1e-2+1e-4,lower);
%                             end
%                         end
%                     end
%                 end

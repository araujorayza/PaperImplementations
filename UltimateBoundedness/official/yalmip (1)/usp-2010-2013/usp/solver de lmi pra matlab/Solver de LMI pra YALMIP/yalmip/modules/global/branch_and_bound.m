...
%     % ********************************************
%     if p.diagonalized
%         b = p.lpcuts(:,1);A = -p.lpcuts(:,2:end);x = sdpvar(4,1);
%         P = projection(polytope(set(A*x < b)+set(p.lb < x < p.ub)),1:2);
%         if isfulldim(P)
%             [A,b] = double(P);
%             x = sdpvar(2,1);plot(set(A*x < b),[],rand(1,3));hold on
%         end
%     end
% 
%     if ~isnan(lower)
%         p = addRangeReductionCut(p,p.lower,upper);
%     end
%      % ********************************************
%     if p.diagonalized
%         b = p.lpcuts(:,1);A = -p.lpcuts(:,2:end);x = sdpvar(4,1);
...
%     if ~isnan(lower)
%         p = addRangeReductionCut(p,lower,upper);
%     end
%     
%     if p.diagonalized
%         b = p.lpcuts(:,1);A = -p.lpcuts(:,2:end);x = sdpvar(4,1);
...
%         [A,b] = double(P);
%          if isfulldim(P)
%         x = sdpvar(2,1);plot(set(A*x < b),[],rand(1,3),100);
%          end
%         if ~isinf(upper)
%             plot(x_min(1),x_min(2),'r*')
%             c = p.c(1:2);
%             d = p.c(3:4);
%          %   ezplot([num2str(p.f,7) '+' num2str(d(1),7) '*x^2+' num2str(d(2),7) '*y*y+' num2str(c(1),7) '*x+' num2str(c(2),7) '*y' '=' num2str(upper,7)])
%             hold on
% 
%         end
%     end
            ...
            case {0,3,4} % No problems (disregard numerical problems)
                ...
                if cost<upper-1e-5
                    ...
                    if numGlobalSolutions < p.options.bmibnb.numglobal
                        ...
                        if 0%oldCount < numGlobalSolutions;
                            xx = sdpvar(4,1);
                            plot(projection(polytope(set(p.lb < xx < p.ub) + set([p.F_struc;p.lpcuts]*[1;xx] > 0)),1:2))
                            p = addRangeReductionCut(p,max(lower,cost),upper);
                            plot(projection(polytope(set(p.lb < xx < p.ub) + set([p.F_struc;p.lpcuts]*[1;xx] > 0)),1:2))
                        end
                    end
                else
...
% p.F_struc = [p.F_struc;p.lpcuts];
% p.K.l = p.K.l + size(p.lpcuts,1);
...
%             d(pos) = 0;
%             p_with_bilinear_cuts.F_struc = [upper+abs(upper)*0.01-f -c' -d';p_with_bilinear_cuts.F_struc];
%             p_with_bilinear_cuts.K.l = p_with_bilinear_cuts.K.l+1;
%             p_with_bilinear_cuts.F_struc = [upper+abs(upper)*0.01-p_with_bilinear_cuts.f -p_with_bilinear_cuts.c';p_with_bilinear_cuts.F_struc];
%             p_with_bilinear_cuts.K.l = p_with_bilinear_cuts.K.l+1;
%         end
%     end
%     p_with_bilinear_cuts.F_struc = [upper+abs(upper)*0.01-p_with_bilinear_cuts.f -p_with_bilinear_cuts.c';p_with_bilinear_cuts.F_struc];
...
if any(p_with_bilinear_cuts.ub+1e-8<p_with_bilinear_cuts.lb)
    ...
    if p.solver.lowersolver.objective.quadratic.convex
        ...
        %         p_with_bilinear_cuts.Q(1:2,1:2) = 0;
        %         p_with_bilinear_cuts.c = p.c;
        %         p_with_bilinear_cuts.c(end-4:end) = 0;
...
function p = addRangeReductionCut(p,lower,upper)

pr = addmcgormick(p);

n = length(p.c);
pr.F_struc = [pr.F_struc;[pr.ub -eye(n);-pr.lb eye(n)]];
pr.K.l = pr.K.l + 2*n;
pr.lb = [];
pr.ub = [];
pr.options.saveduals = 1;
output = callglpk(pr);
cost = pr.c'*output.Primal;

ii = find(output.Dual>1e-10);

% Get the active constraints Ax<b
b = pr.F_struc(ii,1);
A = -pr.F_struc(ii,2:end);

% Range cut Ax > bnew
bnew = (b-(upper-cost)./(output.Dual(ii)));
p.lpcuts = [p.lpcuts;-bnew A];
p.cutState = [p.cutState;ones(length(ii),1)];
